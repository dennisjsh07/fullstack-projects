...
exports.updatePassword = async (req, res, next) => {
    try {
        ...
        const existingPasswordActive = await Forgotpassword.findOne({ requestId: token } );
        console.log('existingPasswordActive >>>>', existingPasswordActive);
        ...
        const userToUpdate = await User.findById(existingPasswordActive.userId);
        if (!userToUpdate) {
            return res.status(404).json({ message: "User not found" });
        }

        await userToUpdate.updateOne({$set: { password: hashedPassword }});

        // Deactivate the token
        await existingPasswordActive.updateOne({ active: false });
...
exports.showPasswordChangePage = async (req, res, next) => {
    try {
        ...
        const existingPasswordActive = await Forgotpassword.findOne({userId: Pid });

        if (existingPasswordActive && existingPasswordActive.active) {
            res.status(200).redirect('http://127.0.0.1:5501/forgot%20password/updatePassword.html');
        } else {
